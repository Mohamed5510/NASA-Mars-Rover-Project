...
def white_color_thresh(img, rgb_thresh=(160, 160, 160)):
...
# Identify pixels above the threshold
# Threshold of RGB < 160 does a nice job of identifying ground pixels only
def Black_color_thresh(img, rgb_thresh=(160, 160, 160)):
    # Create an array of zeros same xy size as img, but single channel
    color_select = np.zeros_like(img[:,:,0])
    # Require that each pixel be above all three threshold values in RGB
    # above_thresh will now contain a boolean array with "True"
...
# Threshold of Rocks by check the yellow color
...
def rock_thresh(img):
    ...
    # determine the mask to get only yellow colors
...
def perception_step(Rover):
    ...
    wraped = perspect_transform(img, source, destination)
    
    # 3) Apply color threshold to identify navigable terrain/obstacles/rock samples
    navigable_terrain = white_color_thresh(wraped, (160, 160, 160))
    obstacles = Black_color_thresh(wraped, (140, 140, 140))
    rock_samples = rock_thresh(wraped)
    ...
    navigable_terrain_x_world, navigable_terrain_y_world = pix_to_world(
        navigable_terrain_xpix, navigable_terrain_ypix, xpos, ypos, yaw, worldmap_size, scale)
    obstacles_x_world, obstacles_y_world = pix_to_world(
        obstacles_xpix, obstacles_ypix, xpos, ypos, yaw, worldmap_size, scale)
    rocks_x_world, rocks_y_world = pix_to_world(
        rocks_xpix, rocks_ypix, xpos, ypos, yaw, worldmap_size, scale)
    ...
    distances, angles = to_polar_coords(navigable_terrain_xpix, navigable_terrain_ypix)
    ...
    if len(rocks_xpix) > 5:
        ...
        rock_distance, rock_angle = to_polar_coords(rocks_xpix, rocks_ypix)
